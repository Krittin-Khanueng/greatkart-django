...
def register(request):
	if request.method == "POST":
		...
		if form.is_valid():
			...
			message = render_to_string("accounts/account_verification_emil.html", {
				'user': user,
				'domain': cureent_site,
...
def login(request):
	if request.method == "POST":
		...
		if user is not None:
			...
			messages.error(request, "InvarIid login credentials")
...
def forgotPassword(request):
	if request.method == "POST":
		...
		if Account.objects.filter(email=email).exists():
			...
			message.success(request, 'Password rest email has been sent to your email address.')
			return redirect('login')
		else:
...
def restpassword_validate(request,uidb64, token):
	...
	if user is not None and default_token_generator.check_token(user, token):
		...
		messages.success(request, "please rest you password")
